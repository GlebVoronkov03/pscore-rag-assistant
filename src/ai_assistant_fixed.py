...
# Добавляем путь для импортов в EXE
if getattr(sys, 'frozen', False):
    # Если запущено как EXE, добавляем путь к библиотекам
...
def check_dependencies():
    """Проверяет наличие необходимых библиотек с подробным выводом"""
    ...
    if missing_libs:
        print(f"\nОтсутствуют библиотеки: {', '.join(missing_libs)}")
        return False
    else:
...
def check_ollama():
    ...
    try:
        ...
        if "qwen2.5:7b" in result.stdout:
            print("OK: Модель qwen2.5:7b найдена")
            return True
        else:
            print("ERROR: Модель qwen2.5:7b не найдена")
            return False
    except:
        print("ERROR: Ollama не установлена или не запущена")
        return False
...
def create_database():
    ...
    if not pdf_files:
        print("ERROR: В папке 'data' нет PDF файлов!")
    ...
    try:
        ...
        # Пробуем новые импорты, потом старые
        try:
            from langchain_huggingface import HuggingFaceEmbeddings
            from langchain_chroma import Chroma
            print("Используются новые версии импортов")
        except ImportError:
            from langchain_community.embeddings import HuggingFaceEmbeddings
            from langchain_community.vectorstores import Chroma
            print("Используются старые версии импортов")
        
        all_documents = []
        for pdf_file in pdf_files:
            print(f"Обработка: {os.path.basename(pdf_file)}")
...
def run_assistant():
    ...
    if not os.path.exists(chroma_path) or not os.listdir(chroma_path):
        print("ERROR: База знаний не найдена!")
        return
    
    try:
        # Пробуем новые импорты
        try:
            from langchain_huggingface import HuggingFaceEmbeddings
            from langchain_chroma import Chroma
        except ImportError:
...
def main():
    ...
    if not check_dependencies():
        print("\nERROR: Не все библиотеки доступны!")
